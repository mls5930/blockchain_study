...
describe("블록 생성 시나리오 - 코인베이스부터 트랜잭션 반영까지", () => {
    ...
    describe("3.단계: 블록 후보 생성후 채굴 그리고 트랜잭션 반영까지", () => {
        ...
        it("트랜잭션 생명주기", () => {
            ...
            const tx = transaction.create(receipt, myUnspentTxOuts)
            // 3. 단계 트랜잭션 풀에서 제거한다
            const txForBlock = [coinbaseTx, tx];
            console.log(transaction.getPool());
            transaction.sync(txForBlock)

            // 4. 단계 unspent.update()를 통해 잔액 상태 반영
            txForBlock.forEach(tx => unspent.update(tx))
